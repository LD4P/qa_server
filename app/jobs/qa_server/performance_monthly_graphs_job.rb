...
# Job to generate monthly performance graphs.
module QaServer
  class PerformanceMonthlyGraphsJob < ApplicationJob
    queue_as :default
    ...
    private
      ...
      def generate_graphs(job_id)
        QaServer.config.monitor_logger.debug("(#{self.class}##{__method__}-#{job_id}) GENERATING MONTHLY performance graphs")
        generate_graphs_for_authorities
        reset_graph_job_id
        QaServer.config.monitor_logger.debug("(#{self.class}##{__method__}-#{job_id}) COMPLETED MONTHLY performance graphs generation")
      end
      ...
      def graph_job_id
        Rails.cache.fetch("QaServer:performance_monthly_graphs-job_id", expires_in: 2.hours, race_condition_ttl: 5.minutes) { false }
      end
      ...
      def graph_job_id=(job_id)
        ...
        current_job_id = Rails.cache.fetch("QaServer:performance_monthly_graphs-job_id", expires_in: 2.hours, race_condition_ttl: 5.seconds) { job_id }

        # current_job_id may be false meaning tests are not currently running; in which case, it is ok to force set job_id
        Rails.cache.fetch("QaServer:performance_monthly_graphs-job_id", expires_in: 2.hours, race_condition_ttl: 30.seconds, force: true) { job_id } unless current_job_id
      end

      # Set job id for performance_monthly_graphs to false indicating that tests are not currently running
      def reset_graph_job_id
        Rails.cache.fetch("QaServer:performance_monthly_graphs-job_id", expires_in: 2.hours, race_condition_ttl: 30.seconds, force: true) { false }
      end
  end
end
